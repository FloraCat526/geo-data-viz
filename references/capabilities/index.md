...
- [Google Maps](google.md)：Data、Advanced Markers、聚合扩展、deck.gl、WebGL。
...
- **第三方**：如 deck.gl；不能称为该地图 SDK 原生功能。
- **组合**：利用公开接口自行组织几何、样式或动画；不是同名内置类。
- **待核实**：当前证据不足，不等于厂商没有此能力，不能进入“已支持”承诺。

“官方文档已核查”仅证明有文档依据；“本地源码已核查”仅证明该源码快照具有接口。实际 SDK 版本、Key、扩展、投影与浏览器验收仍分别记录。本项目附赠 demo 当前内置气泡、呼吸、散点与基础线面；下面的高级路线由 Agent 按任务生成，未因为写入文档就自动加入 demo。

## 效果 → 平台实现路线

各单元格是候选路线，具体输入、依赖和限制以对应平台文档为准。

| 数据表达 | Google | Mapbox | Maptec | 百度 | 高德 | 腾讯 |
|---|---|---|---|---|---|---|
| 散点 / 数量气泡 | Data / AdvancedMarker / 第三方 Scatterplot | 原生 circle | 原生 GeoJSONOverlay 圆样式 | 扩展 PointLayer | 扩展 Loca.PointLayer | 扩展 Dot |
| 呼吸 / 波纹强调 | DOM/Canvas 组合 | 动态图标 / circle 动画组合 | CircleOverlay 组合 | 扩展 RippleLayer | 扩展 ScatterLayer | 扩展 Dot 动画 / Radiation |
...
| 连续热力 | 第三方 deck.gl HeatmapLayer | 原生 heatmap | 原生 HeatmapOverlay | 扩展 HeatmapLayer | 扩展 HeatMapLayer | 扩展 Heat |
| 方格 / 蜂窝统计 | 第三方聚合 / 预计算面 | 预计算 fill / 第三方聚合 | 预计算 GeoJSONOverlay 面 | 扩展 HeatGridLayer / HoneycombLayer | 扩展 GridLayer / HexagonLayer | 扩展 Grid / Hexagon |
...
| OD / 动态飞线 | 第三方 ArcLayer + 自定义动画 | line 组合 / 第三方 ArcLayer | 原生 PulseLinkOverlay（快照） | 扩展 FlyLineLayer | LinkLayer / PulseLinkLayer | 扩展 Arc |
| 有时间的轨迹回放 | 第三方 TripsLayer / 自行时间调度 | 第三方 TripsLayer / line 与 marker 调度 | PointKeyFrameTrack / PolylineKeyFrameTrack | 对应轨迹层按时间契约核实 / 自行调度 | MoveAnimation / PulseLineLayer 仅作方向强调 | 扩展 Trail |
| 指标柱 / 区域拉伸 | 第三方 Column/Polygon 或 WebGL 组合 | 原生 fill-extrusion | PrismOverlay | 扩展 ShapeLayer | 扩展 PrismLayer / PolygonLayer | 扩展 Prism |
| 3D 热力曲面 | 需专门实现，非 deck.gl 二维热力默认能力 | 需自定义，不是 heatmap 内置高度 | Heatmap3DOverlay（快照） | 扩展 HeatmapLayer height，验证版本 | 扩展 HeatMapLayer height | 扩展 Heat height |
