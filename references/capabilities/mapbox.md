...
核查：2026-09-13，Mapbox GL JS 3.x 官方文档与 deck.gl 官方集成说明。锁定任务实际使用的 JS/CSS 版本；文档中的最新示例版本不等于附赠 demo 的版本。附赠 demo 的真实底图、共用 Canvas 气泡/呼吸和状态切换已经使用授权 Token 在本地验证；下表仅有文末列出的原生路线完成实测，其余原生与第三方增强能力尚未逐项验证。
...
| 呼吸、发光点 | `circle` 样式动画组合，或动态图标 + `symbol` | 稳定核心 + 自己驱动的外环；动态图像由 StyleImageInterface 更新 | 是组合效果；图标遮碰撞与叠加顺序需验证，暂停后停止重绘。[动态图标示例](https://docs.mapbox.com/mapbox-gl-js/example/add-image-animated/) |
...
| 方格 / 蜂窝统计 | 预计算 Polygon + 原生 `fill`，或第三方聚合图层 | 固定格网与聚合规则；显示格网统计值 | Mapbox 样式规范没有通用 `hexagon` 图层类型，不发明；第三方需适配。[deck.gl Mapbox 集成](https://deck.gl/docs/api-reference/mapbox/overview) |
...
| 动态飞线 / OD 弧线 | line 几何与动画组合，或第三方 `ArcLayer` | 起终点、流向和流量；是否需要 3D 弧高 | 不是内置 `PulseLinkLayer`；动态脉冲另实现，二维箭头可作为备选。[ArcLayer](https://deck.gl/docs/api-reference/layers/arc-layer) |
| 时间轨迹、尾迹 | 自行调度 source/marker，或第三方 `TripsLayer` | 按实体组织路径与相对时间，统一 currentTime | 不能把日期独立事件串成轨迹；大路径频繁 setData 需实测。[TripsLayer](https://deck.gl/docs/api-reference/geo-layers/trips-layer) |
...
| 自定义 GPU 效果 / 特殊 3D | `CustomLayerInterface` 组合 | 使用任务已验证的投影、WebGL 和坐标转换 | 共用 Canvas 只作为平面方案；自定义层要验证投影/上下文恢复，不能默认球体兼容。[自定义层 API](https://docs.mapbox.com/mapbox-gl-js/api/properties/#customlayerinterface) |
...
点线面统计优先原生数据驱动图层；点太密又需详情时以 cluster/分层可见性提升可读性；固定区域统计用共同预计算面，OD/时间轨迹才引入专门动画或 deck.gl。跨平台时保持原始业务 Feature ID，不能把供应商临时 cluster ID 当实体 ID。
...
跨地图视野实测补充（2026-09-13）：Mapbox→百度→Mapbox 的 89 段新加坡连道验收发现，fitBounds 的 padding 残留及恢复视野时过低的 maxZoom 会改变范围。临时适配可用 `retainPadding:false`，恢复范围不要复用查看单条记录的缩放上限；真实互切需核对中心与跨度，并对不同投影保留合理精度边界。[CameraOptions](https://docs.mapbox.com/mapbox-gl-js/api/properties/#cameraoptions)。本次修正后的原生路线案例完成 12 项检查，不等于附赠 Canvas Playground 的全部互切路径已复验。
